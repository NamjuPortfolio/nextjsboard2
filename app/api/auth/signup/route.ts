...
export const POST = async (
  ...
  if(req.method === 'POST'){
    ...
    const [checkMember] = await db.query<RowDataPacket[]>('select count(*) as cnt from parknamju.member where email = ?', [email]);
    ...
    if(memberCnt > 0){
      ...
      await db.query('insert into parknamju.member (email, password, name) values(?,?,?)',[email, hash, name]);
      const data = {
        email : email,
        password : password
      }
